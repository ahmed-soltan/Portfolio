...
const ProjectsContainer = ({ projects }: { projects: Projects[] }) => {
  ...
  useEffect(()=>{
    setFilteredProjects(projects.filter(project => category==="All" ?projects:project.category === category || project.technologies.includes(category!)))
  },[category])


  return (
    <div className="w-full flex flex-col items-center justify-center 2xl:justify-start mt-5" >
      <CategoryList setCategory={setCategory} category={category}/>
      <div className="w-full flex items-center justify-center 2xl:justify-start gap-4 flex-wrap">
